...
namespace wikia.unit.tests.WikiArticleListTests
{
    ...
    public class ArticleListTests
    {
        ...
        [Test]
        public void Given_A_Null_ArticleListRequestParameters_Should_Throw_ArgumentNullException()
        {
            // Arrange

            // Act
            Func<Task<ExpandedArticleResultSet>> act = () => _sut.ArticleList<ExpandedArticleResultSet>(null, true);
        ...
        public async Task Given_An_ArticleListRequestParameters_The_Response_Items_Collection_Should_Not_Be_Empty()
        {
            ...
            // Act
            var result = await _sut.ArticleList<UnexpandedListArticleResultSet>(new ArticleListRequestParameters("Card_Tips"), false);
